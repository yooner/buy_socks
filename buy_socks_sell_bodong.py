...
# 仓位管理配置
MAX_POSITIONS = 5  # 最大仓位数，可配置为5、8等
...
def _run_backtest_with_trading(stock_code: str, df: pd.DataFrame):
    ...
    carried_position_value = 0  # 上一仓未用完的金额，结转到下一仓
    wait_uptrend_reentry_buy = False  # 上升阶段结束清仓后，等待重新转为上升趋势买入
    wait_downtrend_uptrend_buy = False  # 下降结束未买入后，等待上升趋势确认买入

    # 记录下降趋势的最低点
    downtrend_lowest_price = 0  # 当前下降趋势的最低点
    in_downtrend = False  # 是否在下降趋势中

    # 标记是否已买入；进入新的下降趋势后重置，下降趋势结束才允许重新买入
    ...
    for i in range(len(df)):
        ...
        is_downtrend_end = (
            ('下降阶段结束' in state_transition)
            or (prev_market_state == '下降趋势' and market_state != '下降趋势')
        )

        prev_uptrend_end = False
        curr_uptrend_end = False
        if 'uptrend_end_flag_triggered' in df.columns:
            if i > 0 and pd.notna(df.loc[i-1, 'uptrend_end_flag_triggered']):
                prev_uptrend_end = bool(df.loc[i-1, 'uptrend_end_flag_triggered'])
            if pd.notna(row['uptrend_end_flag_triggered']):
                curr_uptrend_end = bool(row['uptrend_end_flag_triggered'])
        is_uptrend_end_start = curr_uptrend_end and not prev_uptrend_end
        is_uptrend_start = (market_state == '上升趋势' and prev_market_state != '上升趋势')
        if is_downtrend_start:
            wait_uptrend_reentry_buy = False
            wait_downtrend_uptrend_buy = False
        ...
        if position > 0 and not just_bought_today and is_downtrend_start:
            ...
            carried_position_value = 0  # 重置结转金额
            wait_uptrend_reentry_buy = False
            wait_downtrend_uptrend_buy = False
            just_sold_today = True

        # 0.1 上升阶段结束清仓：趋势结束确认当天直接退出，不等下降趋势确认
        if position > 0 and not just_bought_today and not just_sold_today and is_uptrend_end_start:
            sell_price = close_price
            sell_quantity = position
            cash += sell_quantity * sell_price
            total_capital = cash
            profit = (sell_price - first_buy_price) * sell_quantity if first_buy_price > 0 else 0
            profit_pct = (sell_price - first_buy_price) / first_buy_price * 100 if first_buy_price > 0 else 0
            action = f"上升结束清仓@{sell_price:.2f} 盈亏:{profit:+.0f}({profit_pct:+.2f}%)"
            trades.append({
                'day': day_num,
                'date': date_str,
                'action': '上升结束清仓',
            ...
            carried_position_value = 0
            wait_uptrend_reentry_buy = True
            wait_downtrend_uptrend_buy = False
            just_sold_today = True
        ...
        # 2. 第一仓买入逻辑：等待下降趋势结束后再买，不在下降趋势中抄底
        if (
            position == 0
            and is_downtrend_end
            and '本次不买入' not in state_transition
            and not downtrend_buy_triggered
            and not just_sold_today
        ):
            total_capital = cash + position * close_price
            fixed_position_value = total_capital / MAX_POSITIONS
            buy_price = close_price
            buy_budget = fixed_position_value + carried_position_value
            buy_quantity = int(buy_budget / buy_price / 100) * 100
            actual_buy_amount = buy_quantity * buy_price
            next_carried_position_value = buy_budget - actual_buy_amount
            if buy_quantity > 0 and cash >= actual_buy_amount:
                position = buy_quantity
                cash -= actual_buy_amount
                first_buy_price = buy_price
                last_buy_price = buy_price
                position_count = 1
                carried_position_value = next_carried_position_value
                wait_uptrend_reentry_buy = False
                wait_downtrend_uptrend_buy = False
                ...
                trades.append({
                    ...
                    'carried_amount': carried_position_value
                })

        if (
            position == 0
            and is_downtrend_end
            and '本次不买入' in state_transition
            and not just_sold_today
        ):
            wait_downtrend_uptrend_buy = True

        # 2.1 下降结束未买入后，如果后续确认进入上升趋势，重新买第1仓
        if position == 0 and wait_downtrend_uptrend_buy and is_uptrend_start and not just_sold_today:
            total_capital = cash
            fixed_position_value = total_capital / MAX_POSITIONS
            buy_price = close_price
            buy_budget = fixed_position_value + carried_position_value
            ...
            if buy_quantity > 0 and cash >= actual_buy_amount:
                ...
                wait_uptrend_reentry_buy = False
                just_bought_today = True
                action = f"上升确认买入第1仓@{buy_price:.2f} x{position}股"
                trades.append({
                    'day': day_num,
                    'date': date_str,
                    'action': '上升确认买入第1仓',
        ...
        # 2.2 上升阶段结束后，如果未经历下降趋势又重新进入上升趋势，重新买第1仓
        if position == 0 and wait_uptrend_reentry_buy and is_uptrend_start and not just_sold_today:
            total_capital = cash
            fixed_position_value = total_capital / MAX_POSITIONS
            buy_price = close_price
            buy_budget = fixed_position_value + carried_position_value
            ...
            if buy_quantity > 0 and cash >= actual_buy_amount:
                ...
                wait_downtrend_uptrend_buy = False
                just_bought_today = True
                action = f"上升重启买入第1仓@{buy_price:.2f} x{position}股"
                trades.append({
                    'day': day_num,
                    'date': date_str,
                    'action': '上升重启买入第1仓',
        ...
        # 4. 加仓逻辑：每上涨5%加一仓
        if position > 0 and position_count < MAX_POSITIONS and not just_bought_today and not just_sold_today:
            target_price = last_buy_price * (1 + PRICE_INCREASE_PCT)
            if close_price >= target_price:
                # 使用固定的每仓金额，并把上一仓未用完金额结转到本仓
                buy_price = close_price
                buy_budget = fixed_position_value + carried_position_value
                buy_quantity = int(buy_budget / buy_price / 100) * 100
                total_capital = cash + position * close_price
        ...
        # 条件：有持仓，且当前市场状态是下降趋势
        if position > 0 and market_state == '下降趋势' and not just_bought_today and not just_sold_today:
            if i > 0:
                ...
                if pd.notna(macd) and pd.notna(prev_macd):
                    if macd < prev_macd:  # MACD从大变小
                        ...
                        wait_downtrend_uptrend_buy = False
                        just_sold_today = True
                        downtrend_buy_triggered = False  # 等待下一次下降趋势结束后再买入
    ...
    if trades:
        ...
        for trade in trades:
            ...
            elif trade['action'] == '上升确认买入第1仓':
                log_print(f"  上升确认买入记录: {trade['date']} @ {trade['price']:.2f} x {trade['quantity']}股")
            elif trade['action'] == '上升重启买入第1仓':
                log_print(f"  重启买入记录: {trade['date']} @ {trade['price']:.2f} x {trade['quantity']}股")
            elif trade['action'] == '上升结束清仓':
                log_print(f"  清仓记录(上升结束): {trade['date']} @ {trade['price']:.2f} x {trade['quantity']}股 盈亏:{trade['profit']:+.0f}({trade['profit_pct']:+.2f}%)")
