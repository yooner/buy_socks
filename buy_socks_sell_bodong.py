...
# 买入A五日最高条件开关
ENABLE_BUY_A_5DAY_HIGH_CHECK = True  # 设置为True启用：买入A时如果收盘价是五日最高则放弃买入
...
def run_backtest(stock_code: str = STOCK_CODE):
    ...
    # 放弃买入后跟踪状态
    abandoned_buy_price = 0        # 放弃买入时的价格
    abandoned_buy_days = 0         # 放弃买入后的天数计数
    prev_close_price = 0           # 前一天收盘价（用于比较价格趋势）
    ...
    for i in range(len(df)):
        ...
        if pd.notna(volatility):
            ...
            if position > 0:
                ...
                if is_stop_loss_triggered:
                    ...
                    # 重置放弃买入跟踪状态
                    abandoned_buy_price = 0
                    abandoned_buy_days = 0
            ...
            # 放弃买入后的跟踪逻辑：如果后面两天价格都比前一天高，在第二天买入
            if abandoned_buy_days > 0 and position == 0:
                abandoned_buy_days += 1
                # 检查价格是否连续上涨（当前价格 > 前一天价格）
                if close_price > prev_close_price:
                    # 连续上涨，检查是否达到第二天
                    if abandoned_buy_days >= 2:  # 放弃当天(1) + 第一天(2) + 第二天(3)
                        # 第二天价格仍上涨，执行买入
                ...
                # 正常买入逻辑（无待买入时）
                    ...
                    # 条件A买入（全仓）- 主逻辑立即买入，不延迟
                        ...
                        if should_abandon_buy:
                            ...
                            # 设置放弃买入后的跟踪状态
                            abandoned_buy_price = close_price
                            abandoned_buy_days = 1  # 第一天（放弃当天）
                            prev_close_price = close_price
                        else:
                            buy_price = close_price
                            new_position = int(cash / buy_price)
        ...
        # 每天结束时更新前一天收盘价（用于放弃买入后的价格跟踪）
        if abandoned_buy_days == 0:  # 不在放弃买入跟踪期间时更新
            prev_close_price = close_price
